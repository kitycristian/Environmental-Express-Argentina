...
export default function ChemicalSheet() {
  const [, setLocation] = useLocation();
  const [rows, setRows] = useState<ChemicalRow[]>([
    { id: "1", sector: "", puestoTrabajo: "", agenteQuimico: "", numeroCas: "", tiempoExposicion: "", valorMedido: "", unidad: "ppm", cmp: "", cmpCt: "", cumple: "", observaciones: "" }
  ]);
  ...
  const handleImportSectors = () => {
    ...
    if (client && Array.isArray(client.sectors)) {
      const newRows = (client.sectors as string[]).map((sectorName, index) => ({
        ...
        sector: sectorName,
        puestoTrabajo: "", agenteQuimico: "", numeroCas: "", tiempoExposicion: "", valorMedido: "", unidad: "ppm", cmp: "", cmpCt: "", cumple: "", observaciones: ""
      }));
  ...
  const updateRow = (id: string, field: keyof ChemicalRow, value: string) => {
    setRows(rows.map(row => {
      if (row.id === id) {
        const updated = { ...row, [field]: value };
        if (field === 'valorMedido' || field === 'cmp') {
          const medido = parseFloat(updated.valorMedido);
          const limite = parseFloat(updated.cmp);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white border-b px-4 py-2 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-sm font-bold text-gray-800" data-testid="heading-chemical">PROTOCOLO DE MEDICIÓN DE AGENTES QUÍMICOS</h1>
        </div>
          ...
          <div className="space-y-4 py-4">
            {clients.filter(c => c.sectors && (c.sectors as string[]).length > 0).length === 0 ? (
              <div className="p-4 text-center text-muted-foreground border rounded-md bg-muted/50">
                ...
                <p className="text-sm mt-1">Primero agregue sectores a un cliente desde Clientes</p>
              </div>
            ) : (
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-blue-900 text-white">
                  ...
                  <th className="border border-blue-800 px-2 py-1 min-w-[140px]">Agente Químico</th>
                  <th className="border border-blue-800 px-2 py-1 w-24">N° CAS</th>
                  <th className="border border-blue-800 px-2 py-1 w-20">T. Exp. (hs)</th>
                  <th className="border border-blue-800 px-2 py-1 w-24">Valor Medido</th>
                  <th className="border border-blue-800 px-2 py-1 w-20">Unidad</th>
                  <th className="border border-blue-800 px-2 py-1 w-20">CMP</th>
                  <th className="border border-blue-800 px-2 py-1 w-20">CMP-CT</th>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="border p-0">
                      <Input className="h-6 text-xs border-0 rounded-none focus:bg-yellow-50" value={row.agenteQuimico} onChange={(e) => updateRow(row.id, 'agenteQuimico', e.target.value)} data-testid={`input-agente-${index}`} />
                    </td>
                    <td className="border p-0">
                      <Input className="h-6 text-xs border-0 rounded-none focus:bg-yellow-50 text-center" value={row.numeroCas} onChange={(e) => updateRow(row.id, 'numeroCas', e.target.value)} data-testid={`input-cas-${index}`} />
                    </td>
                    <td className="border p-0">
                      <Input className="h-6 text-xs border-0 rounded-none focus:bg-yellow-50 text-center" value={row.tiempoExposicion} onChange={(e) => updateRow(row.id, 'tiempoExposicion', e.target.value)} data-testid={`input-tiempo-${index}`} />
                    </td>
                    <td className="border p-0">
                    ...
                    <td className="border p-0">
                      <Select value={row.unidad} onValueChange={(v) => updateRow(row.id, 'unidad', v)}>
                        <SelectTrigger className="h-6 text-xs border-0 rounded-none" data-testid={`select-unidad-${index}`}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="ppm">ppm</SelectItem>
                          <SelectItem value="mg/m3">mg/m³</SelectItem>
                          <SelectItem value="fibras/cm3">fibras/cm³</SelectItem>
                        </SelectContent>
                      </Select>
                    </td>
                    <td className="border p-0">
                      <Input className="h-6 text-xs border-0 rounded-none focus:bg-yellow-50 text-center" value={row.cmp} onChange={(e) => updateRow(row.id, 'cmp', e.target.value)} data-testid={`input-cmp-${index}`} />
                    </td>
                    <td className="border p-0">
                      <Input className="h-6 text-xs border-0 rounded-none focus:bg-yellow-50 text-center" value={row.cmpCt} onChange={(e) => updateRow(row.id, 'cmpCt', e.target.value)} data-testid={`input-cmpct-${index}`} />
                    </td>
